...
function cssUnicode(cp) {
  return `\\${cp.toString(16).toLowerCase().padStart(4, '0')}`;
}

function iconFontBaseCss(fontName, lc) {
  return `[class^="${lc}-"], [class*=" ${lc}-"] {
...
function iconGlyphCss(lc, valid) {
  return valid.map((d) => `.${lc}-${d.cssClass}:before { content: "${cssUnicode(d.unicode)}"; }`).join('\n');
}

function buildFontCss(fontName, lc, valid) {
  return `/* ${fontName} — Generated by Icon786 */
...
${iconFontBaseCss(fontName, lc)}

${iconGlyphCss(lc, valid)}
`;
}

function buildDemoHtml(fontName, lc, valid, ttfB64) {
  const firstUni = valid[0]?.unicodeStr || 'E000';
  const lastUni = valid[valid.length - 1]?.unicodeStr || firstUni;
  const glyphRules = iconGlyphCss(lc, valid);
...
body.font-loading .grid{opacity:.35}
...
.font-status{font-size:12px;color:#6B7280;margin-top:8px}
.font-status.error{color:#dc2626}
...
.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:14px;transition:opacity .25s}
...
.glyph{font-size:40px;color:#E8395A;display:block;margin-bottom:10px;line-height:1;min-height:48px;width:100%}
...
<header>
  ...
  <div class="meta">
    <div>Generated by <strong>Icon786</strong> — Free Icon Library</div>
    <div>${valid.length} icons · Unicode U+${firstUni}–U+${lastUni}</div>
  </div>
  <div class="badge">✓ Fonts embedded · Works offline</div>
  <div class="font-status" id="fontStatus">Loading icon font…</div>
</header>

<div class="usage">
  <h2>How to use in your project</h2>
  <div class="steps">
    <div class="step"><div class="step-label">1 — Extract ZIP</div><p>Unzip all files into one folder before opening this demo.</p></div>
    <div class="step"><div class="step-label">2 — Add CSS</div><p>Link <code>${fontName}.css</code> in your HTML <code>&lt;head&gt;</code></p></div>
    <div class="step"><div class="step-label">3 — Copy font files</div><p>Keep <code>${fontName}.ttf</code> &amp; <code>${fontName}.woff</code> next to your CSS file.</p></div>
    <div class="step"><div class="step-label">4 — Use classes</div><p>Add an icon: <code>&lt;i class="${lc}-home"&gt;&lt;/i&gt;</code></p></div>
  </div>
</div>
...
${valid.map((d) => `<div class="item" data-n="${d.name} ${d.cssClass}">
  <i class="glyph ${lc}-${d.cssClass}" aria-hidden="true"></i>
...
<footer><p>Made with ♥ by <a href="https://icon786.com">Icon786</a> · Open <strong>${fontName}-demo.html</strong> from an extracted folder</p></footer>

<script>
var FONT_NAME='${fontName.replace(/'/g, "\\'")}';
var TTF_B64='${ttfB64}';

function filter(q){var n=0;document.querySelectorAll('.item').forEach(function(el){var s=el.dataset.n.toLowerCase().includes(q.toLowerCase());el.classList.toggle('hide',!s);if(s)n++;});document.getElementById('empty').classList.toggle('show',n===0&&q.length>0);}
function cp(t,b){navigator.clipboard.writeText(t).then(function(){var o=b.textContent;b.textContent='✓ copied';b.style.cssText='background:#f0fdf4;border-color:#bbf7d0;color:#166534';setTimeout(function(){b.textContent=o;b.style.cssText='';},1500);});}

(function loadEmbeddedFont(){
  var status=document.getElementById('fontStatus');
  var ttfUrl='data:application/x-font-ttf;base64,'+TTF_B64;
  var css='@font-face{font-family:"'+FONT_NAME+'";src:url("'+ttfUrl+'") format("truetype");font-weight:normal;font-style:normal;font-display:swap;}';
  ...
  function markReady(){
    ...
    if(status) status.textContent='Icon font loaded — ${valid.length} icons ready';
  }
  ...
  if(window.FontFace){
    try{
      var face=new FontFace(FONT_NAME,'url('+ttfUrl+')');
  ...
  document.fonts&&document.fonts.ready?document.fonts.ready.then(markReady).catch(function(){markReady();}):setTimeout(markReady,300);
})();
</script>
</body>
</html>`;
...
router.post('/generate', async (req, res) => {
  try {
    ...
    const lc = fontName.toLowerCase().replace(/[^a-z0-9-]/g, '');

    // 4. CSS (references hosted font files — use on a web server or with extracted files)
    const css = buildFontCss(fontName, lc, valid);

    // 5. Self-contained HTML demo (TTF embedded via JS — works when opened locally)
    const html = buildDemoHtml(fontName, lc, valid, ttfB64);
...
## Preview icons (important)
1. **Extract the full ZIP** into a folder (do not open files inside the ZIP).
2. Open **\`${fontName}-demo.html\`** from that folder — icons are embedded and work offline.
...
| \`${fontName}-demo.html\` | Self-contained demo (icons work offline) |
...
<i class="${lc}-home"></i>
\`\`\`
Keep \`.css\`, \`.ttf\`, and \`.woff\` in the same folder (or adjust paths in CSS).
