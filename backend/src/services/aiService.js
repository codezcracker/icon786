...
function resolveCredentials(provider, userKey, userBaseUrl, model) {
  ...
  if (provider === 'server') {
    ...
    return {
      ...
      apiKey: key,
      model: model || serverCfg.defaultModel,
    };
  }
...
Rules: 24x24 viewBox, single color via currentColor, simple paths only, no scripts, no external refs.`;
  ...
  if (!content) {
    throw new Error(
      'AI is not configured. For local dev: open AI settings (gear icon) and add your API key, '
      + 'or set OPENAI_API_KEY in backend/.env and restart the server.'
    );
  }
...
function getPublicConfig() {
  return {
    ...
    models: {
      ...
      huggingface: ['meta-llama/Meta-Llama-3-8B-Instruct', 'HuggingFaceH4/zephyr-7b-beta', 'mistralai/Mistral-7B-Instruct-v0.3'],
      custom: ['gpt-4o-mini'],
    },
  };
}
