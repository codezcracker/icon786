...
export default function AiApiConfigModal({ open, onOpenChange, serverAiEnabled = false }) {
  ...
  const handleSave = () => {
    if (!config) return;
    const errors = validateApiConfig(config, { serverAiEnabled });
                  ...
                  <ul className="ai-api-features">
                    ...
                    {serverAiEnabled ? (
                      ...
                      <li className="ai-api-features__warn">
                        <span>Local dev: add your API key below, or set OPENAI_API_KEY in backend/.env</span>
                      </li>
                    )}
                  </ul>
                  {!serverAiEnabled && (
                    <>
                      <label className="form-label" htmlFor="ai-server-api-key">
                        API Key <span className="ai-api-required">*</span>
                      </label>
                      <input
                        id="ai-server-api-key"
                        type="password"
                        ...
                        placeholder="sk-… (OpenAI, Groq, or OpenRouter key)"
                        autoComplete="off"
                      />
                      {errorsByField.apiKey && <p className="ai-api-field-error">{errorsByField.apiKey}</p>}
                      <p className="ai-api-panel__hint">
                        Use an OpenAI key here, or switch provider to Groq / OpenRouter for other keys. Stored in your browser only.
                      </p>
                    </>
                  )}
                </>
