...
export const PROVIDER_INFO = {
  ...
  huggingface: {
    id: 'huggingface',
    name: 'Hugging Face',
    description: 'HF Inference API (hf_ keys)',
    baseUrlEditable: false,
    modelEditable: true,
...
export function hasAiCredentials(config, serverAiEnabled = false) {
  const id = config?.selectedProvider || 'server';
  const pcfg = getSelectedProviderConfig(config);
  const info = PROVIDER_INFO[id];
  if (info?.isDefault) {
    return Boolean(serverAiEnabled || pcfg?.apiKey?.trim());
  }
  return Boolean(pcfg?.apiKey?.trim());
}

export function validateApiConfig(config, { serverAiEnabled = false } = {}) {
  const errors = [];
  ...
  if (info.isDefault) {
    if (!serverAiEnabled && !pcfg?.apiKey?.trim()) {
      errors.push({
        field: 'apiKey',
        message: 'Add your API key for local development, or set OPENAI_API_KEY in backend/.env',
      });
    }
    return errors;
  }
